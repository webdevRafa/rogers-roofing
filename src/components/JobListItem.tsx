  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="text-base font-semibold text-[var(--color-text)]">{job.address?.fullLine}</div>
          <div className="text-sm text-[var(--color-muted)]">
            Last updated: {last ? new Date((last as any)?.toDate ? (last as any).toDate() : last).toLocaleString() : "—"}
          </div>
          <div className="mt-1 inline-flex items-center gap-2 text-xs uppercase tracking-wide text-[var(--color-muted)]">
            <span className="rounded-full px-2 py-0.5 border border-white/20">{job.status}</span>
          </div>
        </div>
        <div className="text-right">
          <div className="text-sm text-[var(--color-muted)]">Net Profit</div>
          <div className="text-xl font-bold text-[var(--color-text)]">{formatCurrency(net)}</div>
        </div>
      </div>
    </Link>
  );
