...
const STATUS_OPTIONS: JobStatus[] = ["draft", "pending", "active", "invoiced", "paid", "closed", "archived"];
...
function addr(a: Job["address"] | null | undefined) {
  if (typeof a === "string") return { display: a, line1: a, city: "", state: "", zip: "" };
  const obj: Record<string, unknown> = (a as unknown as Record<string, unknown>) ?? {};
  ...
  const display = pickString(obj, ["fullLine", "full", "formatted", "label", "text"]) || line1;
...
export default function JobsPage() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => setJobs(snap.docs.map((d) => d.data())));
    return () => unsub();
  }, []);
  ...
  const filteredJobs = useMemo(() => {
    ...
    const startMs = hasStart ? new Date(startDate + "T00:00:00").getTime() : null;
    ...
    return jobs.filter((j) => {
      ...
      if (term.length > 0) {
        ...
        const haystack = [a.display, a.line1, a.city, a.state, a.zip].filter(Boolean).join(" ").toLowerCase();
  ...
  const totalNet = useMemo(
    () => filteredJobs.reduce((acc, j) => acc + (j.computed?.netProfitCents ?? 0), 0),
    [filteredJobs]
  );
  ...
  return (
    ...
    >
      ...
      <motion.header
        ...
        {...fadeUp(0)}
      >
        <h1 className="text-xl sm:text-2xl font-bold text-[var(--color-text)]">Jobs</h1>
              ...
              <div className="mt-2 sm:absolute sm:right-0 sm:mt-2 w-full sm:w-80 rounded-xl border border-[var(--color-border)] bg-white p-2 shadow-lg">
                ...
                {searchTerm && (
                  <div className="mt-1 text-xs text-[var(--color-muted)]">
                    Filtering by: <span className="font-medium">{searchTerm}</span>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="flex w-full flex-col gap-2 sm:flex-row sm:gap-3">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={createJob}
                disabled={loading}
                className="w-full sm:w-auto rounded-lg bg-[var(--color-text)] text-[var(--btn-text)] px-3 py-2 text-sm hover:bg-[var(--btn-hover-bg)] disabled:opacity-50"
              >
      ...
      >
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-[1fr_1fr_auto_auto_auto_auto] sm:items-end">
          <div className="flex-1">
            <label className="mb-1 block text-xs text-[var(--color-muted)]">Start date</label>
            <input
              type="date"
              value={startDate}
              onChange={(e) => {
          ...
          <div className="flex-1">
            <label className="mb-1 block text-xs text-[var(--color-muted)]">End date</label>
            <input
              type="date"
              value={endDate}
              onChange={(e) => {
        ...
        <p className="mt-2 text-xs text-[var(--color-muted)]">
          Filters use each job&apos;s <strong>last updated</strong> date (falls back to created date).
        </p>
      </motion.section>

      {/* Totals */}
      <motion.div className="mb-3 text-sm font-semibold text-[var(--color-text)]" {...fadeUp(0.1)}>
        Total net across {filteredJobs.length} job{filteredJobs.length === 1 ? "" : "s"}:{" "}
      ...
      <div className="grid gap-3 sm:hidden">
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="font-medium text-[var(--color-text)]">{a.display || "—"}</div>
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 text-sm">
                <div className="rounded-lg bg-white/60 p-2">
                  <div className="text-[var(--color-muted)] text-xs">Earnings</div>
                  <div className="font-medium">
                    <CountMoney cents={job.earnings?.totalEarningsCents ?? 0} />
                  </div>
                </div>
                <div className="rounded-lg bg-white/60 p-2 text-right">
                  <div className="text-[var(--color-muted)] text-xs">Expenses</div>
                  <div className="font-medium">
                    <CountMoney cents={job.computed?.totalExpensesCents ?? 0} />
                  </div>
                </div>
        ...
        {filteredJobs.length === 0 && (
          <div className="text-center text-[var(--color-muted)]">No jobs match the current filters.</div>
        )}
      </div>
              ...
              {filteredJobs.map((job, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="text-[var(--color-text)]">{fmtDateTime(job.updatedAt)}</div>
                      <div className="text-xs text-[var(--color-muted)]">
